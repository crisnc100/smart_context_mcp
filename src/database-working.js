...
/**
 * Working database implementation with proper SQL.js methods
 * This fixes all the database issues found in testing
 */

let db = null;
const dbPath = './data/context.db';

// Simple operation queue to prevent concurrent issues
...
function queueOperation(operation) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(processQueue, 0); // Process on next tick
  });
}
...
function createTables() {
  ...
  try {
    ...
    console.warn('Failed to insert default patterns:', error.message);
  }
}

// Enhanced database object with proper SQL.js methods
const enhancedDb = {
  prepare: (sql) => {
    return {
      ...
      // For getting multiple rows, use exec and parse results
      all: (...params) => queueOperation(() => {
        if (!db) throw new Error('Database not initialized');
        
        if (params.length > 0) {
          // For parameterized queries, we need to use a different approach
          // Since SQL.js doesn't have a direct .all() method for prepared statements
          const stmt = db.prepare(sql);
          const results = [];
          stmt.bind(params);
